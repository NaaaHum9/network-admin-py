...
@routers_bd.route("/<host>/interfaces/<path:interfaz>/estado", methods=["POST"])
...
@routers_bd.route("/<host>/interfaces/<path:interfaz>/estado", methods=["DELETE"])
def detener_traps_interfaz(hot, interfaz):
    ok = traps_service.stop_trap_interface_trap_status(host, interfaz)
    if ok:
        estado = traps_service.get_interface_trap_status(host, interfaz)
        return jsonify({
...
@routers_bd.route("/<host>/interfaces/<path:interfaz>/grafica_traps", methods=["GET"])
